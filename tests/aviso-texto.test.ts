...
describe('aviso de caso calificado', () => {
  ...
  it('el asunto basta para saber de quién es sin abrirlo', () => {
    expect(aviso.titular).toContain('María Fernanda Solís')
  })

  it('lleva lo necesario para actuar: a quién llamar y a qué número', () => {
    expect(aviso.cuerpo).toContain('55 4433 2211')
    expect(aviso.cuerpo).toContain('Ciudad de México')
    expect(aviso.cuerpo).toContain('11 días')
    expect(aviso.cuerpo).toContain('SL-000042')
  })
  ...
  it('NO saca el relato del despido al correo', () => {
    ...
    expect(aviso.cuerpo).not.toContain('liquidación')
    expect(aviso.cuerpo).toContain('está en el portal')
  })

  it('avisa cuando NO hay descripción, que es lo que cambia la llamada', () => {
    const sin = avisoCasoCalificado({ ...LEAD, description: '' }, ID, URL)
    expect(sin.cuerpo).toContain('No escribió descripción')
  })

  it('no promete lo que ningún abogado ha decidido', () => {
    expect(aviso.cuerpo).toContain('cumple los criterios iniciales')
    expect(aviso.cuerpo.toLowerCase()).not.toContain('aceptado')
  })

  it('enlaza al expediente, y se aguanta sin dominio configurado', () => {
    expect(aviso.cuerpo).toContain(`${URL}/portal/${ID}`)
    expect(avisoCasoCalificado(LEAD, ID, '').cuerpo).not.toContain('/portal/')
  })
})
...
describe('aviso de llamada inmediata', () => {
  ...
  it('el asunto empieza por la acción, no por el hecho', () => {
    ...
    expect(aviso.titular.startsWith('LLAMAR AHORA')).toBe(true)
    expect(aviso.titular).toContain('10 min')
    expect(aviso.titular).toContain('55 4433 2211')
  })

  it('recuerda que la pantalla ya se lo prometió', () => {
    expect(aviso.cuerpo).toContain('se lo prometió')
  })
})
...
describe('aviso de llamada agendada', () => {
  ...
  it('dice cuándo la pidió', () => {
    expect(aviso.titular).toContain('03/09/2026')
    expect(aviso.titular).toContain('4:00 pm')
  })

  it('no la convierte en una cita confirmada por el despacho', () => {
    expect(aviso.cuerpo).toContain('no una cita confirmada por el despacho')
  })
})
